...
export default function PoleChudesTestGame({ onClosePanel, layout = "page", onPauseBookHymn }: PoleChudesTestGameProps = {}) {
  ...
  const onSpinComplete = useCallback(
    async (spinResult: SpinResult, attempt: number, usedSnapshot: Record<string, Set<string>>) => {
      ...
      // Кнопки результата не должны ждать окончания длинных аудиодорожек.
      // Сами игровые эффекты сохраняем полностью, но воспроизводим асинхронно.
      setResultReady(true);
      setBusy(false);
        ...
        <AnimatePresence mode="wait">
          ...
          {stage === "GAME" && (
            ...
            >
              ...
              <div className="-mt-16 relative z-20 pointer-events-auto flex max-w-full flex-wrap items-center justify-center gap-1 justify-self-center sm:-mt-14 sm:gap-1.5">
