...
const LetterGlitch: React.FC<LetterGlitchProps> = ({
  ...
  const handleSmoothTransitions = () => {
    ...
    letters.current.forEach(letter => {
      if (letter.colorProgress < 1) {
        ...
        const startRgb = hexToRgb(letter.color.startsWith('rgb') ? rgbToHex(letter.color) : letter.color);
  ...
  // Helper to handle the case where letter.color is already rgb()
  ...
  const containerStyle: React.CSSProperties = {
    ...
    width: '100%',
    height: '100%',
    backgroundColor: 'transparent', // Modified from #000 to allow layering
    overflow: 'hidden'
  };
